...
describe('PaymentService', () => {
  ...
  describe('createPayment', () => {
    it('deve criar uma transação de cartão usando InfinitePay', async () => {
      ...
      // Mock do provider InfinitePay para retornar sucesso
      infiniteProvider.createCardTransactionWithInfinite.mockResolvedValue({
        success: true,
        status: 'pending',
        provider: 'infinity',
        providerReference: 'inf-link-123',
        raw: { id: 'inf-link-123', url: 'http://link' }
      });
      ...
      // 2. Verifica se chamou o provedor InfinitePay com os dados certos
      expect(infiniteProvider.createCardTransactionWithInfinite).toHaveBeenCalledWith(expect.objectContaining({
        amount_cents: 5000,
      ...
      expect(result.provider).toBe('infinity');
    });
  });
});